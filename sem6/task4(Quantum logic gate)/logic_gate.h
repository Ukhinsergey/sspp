...
complexd *TwoQubitsEvolution(complexd *a, int n, int q1, int q2, complexd u[4][4], long powproc, int rank)
{
    ...
    return b;
        ...
        MPI_Sendrecv(a, vec_length * 2, MPI_DOUBLE, needrank, 0, vec00, vec_length * 2, MPI_DOUBLE, needrank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

        int value01 = start & ~pow2q1 | pow2q2;
        needrank = value01 / vec_length;
        complexd *vec01 = new complexd[vec_length];
        MPI_Sendrecv(a, vec_length * 2, MPI_DOUBLE, needrank, 0, vec01, vec_length * 2, MPI_DOUBLE, needrank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

        int value10 = (start | pow2q1) & ~pow2q2;
        needrank = value10 / vec_length;
        complexd *vec10 = new complexd[vec_length];
        MPI_Sendrecv(a, vec_length * 2, MPI_DOUBLE, needrank, 0, vec10, vec_length * 2, MPI_DOUBLE, needrank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
