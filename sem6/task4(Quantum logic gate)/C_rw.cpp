...
int main(int argc, char **argv) {
    ...
    */
    ...
    cout << dlina << endl;
    */
    complexd *b = C_rw(a, n, q1, q2, powproc, rank, phi);
    ...
    int offset = sizeof(int) + vec_length * 2 * rank * sizeof(double);
    if( rank == 0) {
        MPI_File_open(MPI_COMM_SELF, argv[6], MPI_MODE_WRONLY | MPI_MODE_CREATE, MPI_INFO_NULL, &file);
        MPI_File_write(file, &n, 1, MPI_INT, &status);
        MPI_File_close(&file);
    }
    MPI_File_open(MPI_COMM_WORLD, argv[6], MPI_MODE_WRONLY | MPI_MODE_CREATE, MPI_INFO_NULL, &file);
    MPI_File_set_view(file, offset, MPI_DOUBLE, filetype, "native", MPI_INFO_NULL);
