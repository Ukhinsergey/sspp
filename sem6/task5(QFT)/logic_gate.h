...
complexd *qubit_transform(complexd *a, complexd*b, int  n, complexd u[2][2], int  k,
...
complexd *TwoQubitsEvolution(complexd *a, complexd *b, int n, int q1, int q2,
...
complexd *Hadamar(complexd *a, complexd *b, int  n, int  k, int powproc, int rank) {
    complexd u[2][2];
    u[0][0] = u[0][1] = u[1][0] = 1.0 / sqrt(2.0);
    u[1][1] = -u[0][0];
    return qubit_transform(a, b, n, u, k, powproc, rank);
}

complexd *N_hadamar(complexd *a, complexd *b, int  n, int powproc, int rank) {
    complexd u[2][2];
    u[0][0] = u[0][1] = u[1][0] = 1.0 / sqrt(2.0);
    u[1][1] = -u[0][0];
    long vec_length = 1 << (n - powproc);
    complexd *temp1 = new complexd[vec_length];
    complexd *temp2 = new complexd[vec_length];
    qubit_transform(a, temp1, n, u, 1, powproc, rank);
    for (int i = 2 ; i <= n; ++i) {
        qubit_transform(temp1, temp2, n, u, i, powproc, rank);
        complexd *swap = temp1;
        temp1 = temp2;
        temp2 = swap;
    }
...
complexd *Not(complexd *a, complexd *b, int  n, int  k, int powproc, int rank) {
    complexd u[2][2];
    u[0][0] = u[1][1] = 0;
    u[0][1] = u[1][0] = 1;
    return qubit_transform(a, b, n, u, k, powproc, rank);
...
complexd *Rw(complexd *a, complexd *b, int  n, int  k, int powproc, int rank, double phi) {
    complexd u[2][2];
    u[0][0] = 1;
    u[0][1] = u[1][0] = 0;
...
complexd *C_not(complexd *a, complexd *b, int  n, int  q1, int q2, long powproc, int rank) {
    complexd u[4][4];
    u[0][0] = u[1][1] = u[2][3] = u[3][2] = 1;
    u[0][1] = u[0][2] = u[0][3] = u[1][0] = u[1][2] = u[1][3] = u[2][0] =
    u[2][1] = u[2][2] = u[3][0] = u[3][1] = u[3][3] = 0;
    return TwoQubitsEvolution(a, b, n, q1, q2, u, powproc, rank);
}


complexd *C_rw(complexd *a, complexd *b, int  n, int  q1, int q2, long powproc, int rank,
    double phi) {
